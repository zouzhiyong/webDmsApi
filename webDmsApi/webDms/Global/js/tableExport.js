  ...
  $.fn.extend({
    tableExport: function (options) {
      ...
      if ( defaults.type == 'csv' || defaults.type == 'tsv' || defaults.type == 'txt' ) {
        ...
        //手动修改内容，用于JSON生成EXCEL
        if ((defaults.jsondata || null) != null) {
            var objBody = [];
            if ((defaults.jsondata.data || null) == null || (defaults.jsondata.header || null) == null) return;

            defaults.jsondata.data.map(function (item, index) {
                var objHead = [];
                var objBody = [];
                for (var _item in item) {
                    if (index == 0) {
                        defaults.jsondata.header.map(function (item_name) {
                            if (item_name.code == _item) {
                                objHead.push(item_name.name);
                            }
                        })
                    }
                    objBody.push(item[_item]);
                }
                if (index == 0) {
                    data.push(objHead);
                } else {
        ...
        try {
      ...
      else if (defaults.type == 'excel-json' && defaults.jsondata != null) {
          /*
                以下适应于json对象的导出
                json对象的格式要求
                var data = {
                    "header": [
                        { "code": "col1", "name": "列一", "datatype": "number"},
                        { "code": "col2", "name": "列二", "datatype": "datetime" }
                    ],
                    "data": 
                        [
                            { "col1": "好好", "col2": "不好" },
                            { "col1": "1233333", "col2": "2017-08-04" }
                        ]
                };
                调用方式： $(this).tableExport({ type: 'excel-json', jsondata: data });
                */
          ...
          //设置表头  
          var row = "<tr>";
          $.each(defaults.jsondata.header, function (i, item) {
              row += "<th>" + item.name + '</th>';
          });
          //换行  
          excel += row + "</tr></thead><tbody>";
          //设置数据 excel格式问题 http://www.cnblogs.com/lx1137433756/p/6423517.html
          $.each(defaults.jsondata.data, function (i, item_d) {
              var row = "<tr>";
              $.each(defaults.jsondata.header, function (j, item_t) {
                  var datatype = item_t.datatype;//列数据类型，暂时不用
                  var value = item_d[item_t.code]
          ...
          docFile += "</xml>";
          docFile += "<![endif]-->";
          docFile += "<style>br {mso-data-placement:same-cell;} table {border: 1px}</style>";
          docFile += "</head>";
          docFile += "<body>";
          docFile += excel;
          docFile += "</body>";
